...
-(BOOL)addRelationshipByKeySourse:(nonnull NSString *)keyS andKeyTarget:(nonnull NSString *)keyT;

//这是一个万全的方法 插入是 不会报错的(除了参数有点多。。)
-(void)addRelationshipByObject:(nonnull id)source andSourceKey:(nonnull NSString*)keyS and:(nonnull id)target andTargetKey:(nonnull NSString*)keyTar;//插入对应关系
